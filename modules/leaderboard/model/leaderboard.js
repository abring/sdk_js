...
var initLeaderboard = function () {
    ...
    $("#"+abring.params.leaderboard_parent_id).html(leaderboardView);
};

var leaderboard_loading_start = function () {

    abringPageShow("loading","loading leaderboard data.");
    abringLoadingStart();

};
var leaderboard_loading_finish = function () {
    abringPageHide();
};
var leaderboard_show_page = function () {
    abringPageShow("leaderboard");
};

var abringLeaderboardShow = function() {
    abring.params.leaderboard_loading_start_function();
    var method = "leaderboard/get-all";
    var data = {};
    callAbringWithFileUpload(method,data,loadLeaderboardSuccess,loadLeaderboardFail);
};

var loadLeaderboardSuccess = function (leaderboard_data_tmp) {
    if(leaderboard_data_tmp==undefined || leaderboard_data_tmp==false ||leaderboard_data_tmp=="" )
        return loadLeaderboardFail();
    ...
    $("#"+abring.params.leaderboard_parent_id+" .leaderbord_display").html(leaderboard_data_html);
    abring.params.leaderboard_loading_finish_function();
    abring.params.leaderboard_show_page_function();
};
var loadLeaderboardFail = function () {
    abring.params.leaderboard_loading_finish_function();
    // abringLoadingFinish();
    abringPageShow("error",abring.params.last_error);
    return false;
};
